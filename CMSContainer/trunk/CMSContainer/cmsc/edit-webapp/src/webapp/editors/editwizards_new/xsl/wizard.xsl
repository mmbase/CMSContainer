    ...
    <xsl:if test="command[@name=&apos;add-item&apos;]">
      ...
      <xsl:for-each select="command[@name=&apos;articlesearch&apos;]">
		...
		function articlesearch(){
			...
			window.open('../../../../editors/repository/select/index.jsp?action=select&amp;position=wizard&amp;initsearchtext=' + searchtext + '&amp;type=article', 'contentselector', getPopupPositionProps(1000,550)+',status=yes,toolbar=no,titlebar=no,scrollbars=yes,resizable=yes,menubar=no');
		}
        </script>
        <table class="searchcontent">
          <tr>
